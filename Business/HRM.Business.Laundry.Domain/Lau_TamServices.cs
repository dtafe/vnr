...
{
    public class Lau_TamServices : BaseService
    {
        ...
        public Guid CreateComputingTask(Guid userID, DateTime dateFrom, DateTime dateTo)
        {
            ...
            using (var context = new VnrHrmDataContext())
            {
                ...
                task.Type = AsynTask.Download_TimeLog_LMS.ToString();
                task.Summary = "LMS TimeLog: " + dateFrom.ToString("dd/MM/yyyy") + " - " + dateTo.ToString("dd/MM/yyyy");
            ...
            using (var context = new VnrHrmDataContext())
            {
                ...
                if (listProfileID != null && listProfileID.Count() > 0)
                {
                    ...
                    foreach (var listSplitProfileID in listProfileID.Chunk(pageSize))
                    {
                        ...
                        if (IsConnected(AppConfig.HRM_SYS_LAU_TAMSCANLOG_1_, true, ref serverTamLogConfig1))
                        {
                            SyncTAMLog(userID, isExcept, dateFrom, dateTo, serverTamLogConfig1, listSplitProfileID.ToList());
                        }

                        if (IsConnected(AppConfig.HRM_SYS_LAU_TAMSCANLOG_2_, true, ref serverTamLogConfig2))
                        {
        ...
        private void DeleteTAMLog(IUnitOfWork unitOfWork, DateTime dateFrom, DateTime dateTo,
            List<LMS_TamScanLogLMS> listTAMScanLog, Guid userID)
        {
            //Honda dùng 2 db chấm công nên chỉ xóa dữ liệu trùng
            Boolean checkDeleteIdentical = true;

            List<LMS_TamScanLogLMS> lstDelScan = new List<LMS_TamScanLogLMS>();
            String status = TAMScanStatus.E_LOADED.ToString();

            List<LMS_TamScanLogLMS> lstScanDB = unitOfWork.CreateQueryable<LMS_TamScanLogLMS>(Guid.Empty,
            ...
            if (checkDeleteIdentical)
            {
                lstDelScan = lstScanDB.Where(del => listTAMScanLog.Any(tam => del.TimeLog == tam.TimeLog
                    && ((!string.IsNullOrWhiteSpace(tam.Code) && del.Code == tam.Code)
                    || (!string.IsNullOrWhiteSpace(tam.CodeEmp) && del.CodeEmp == tam.CodeEmp)))).ToList();
            }
            else
            {
                var lstCardCode = listTAMScanLog.Where(d => !string.IsNullOrWhiteSpace(d.Code)).Select(d => d.Code).Distinct().ToList();
                var lstCodeEmp = listTAMScanLog.Where(d => !string.IsNullOrWhiteSpace(d.CodeEmp)).Select(d => d.CodeEmp).Distinct().ToList();
                lstDelScan = lstScanDB.Where(del => lstCardCode.Contains(del.Code) || lstCodeEmp.Contains(del.CodeEmp)).ToList();
            }

...
namespace HRM.Infrastructure.Security
{
    ...
    {
        /// <summary>
        /// Chuyển một queryable thành list - tương đương hàm ToList.
        /// </summary>
        /// <param name="queryable"></param>
        /// <returns></returns>
        public static IList GetList(this IQueryable queryable)
        {
            Type type = typeof(List<>).MakeGenericType(queryable.ElementType);
            return (IList)Activator.CreateInstance(type, queryable);
        }

        private static object FirstOrDefault(this IQueryable queryable)
        {
            IList listEntity = queryable.Take(1).GetList();
                ...
                if (listInfo != null)
                {
                    ...
                    foreach (var fieldInfo in listInfo)
                    {
                        ...
                        if (model.HasProperty(fieldName))
                        {
                            ...
                            #region So sánh hai giá trị mới nhau

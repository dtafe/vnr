...
{
    public class Cat_ComputingTypeController : ApiController
    {
        #region UserLogin
        ...
        public Cat_NameEntityModel GetById(Guid id)
        {
            ...
            var entity = service.GetByIdUseStore<Cat_NameEntityEntity>(id, ConstantSql.hrm_hr_sp_get_ProfileComputingById, ref status);
            if (entity!=null)
            {
        ...
        {
            model.NameEntityType = "E_COMPUTING_TYPE";
            ActionService service = new ActionService(UserLogin);

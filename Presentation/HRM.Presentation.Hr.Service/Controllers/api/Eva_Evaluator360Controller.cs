...
{
    public class Eva_Evaluator360Controller : ApiController
    {
        #region UserLogin
        ...
        [System.Web.Mvc.RouteAttribute("api/Eva_Evaluator360")]
        public Eva_EvaluatorModel Post([Bind]Eva_EvaluatorModel model)
        {
            #region Validate

            string message = string.Empty;
            var checkValidate = HRM.Business.Main.Domain.ValidatorService.OnValidateData<Eva_EvaluatorModel>(model, "Eva_Evaluator360", ref message);
            if (!checkValidate)
            {
            ...
            evaluatorService.AddEvaluator360s(evaluatorEntity);
            return model;

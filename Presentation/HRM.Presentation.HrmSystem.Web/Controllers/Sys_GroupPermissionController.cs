...
namespace HRM.Presentation.HrmSystem.Web.Controllers
{
    public class Sys_GroupPermissionController : BaseController
    {
        ...
        {
            var isAccess = CheckPermission(UserId, PrivilegeType.View, ConstantPermission.Sys_GroupPermission);
            if (!isAccess)
            {
            ...
            var service = new RestServiceClient<IEnumerable<Sys_GroupPermissionModel>>();
            ...
            if (listAvailablePermissions.Any())
            {
                ...
                {
                    var groupService = new RestServiceClient<Sys_GroupModel>();
        ...
        {
            var isAccess = CheckPermission(UserId, PrivilegeType.Create, ConstantPermission.Sys_GroupPermission);
            if (!isAccess)
            {
                return PartialView("AccessDenied");
            }
            if (!string.IsNullOrEmpty(id))
            {
                ...
                var service = new RestServiceClient<Sys_GroupPermissionModel>();
                service.SetCookies(Request.Cookies, hrm_Sys_Service);
        ...
        {
            var isAccess = CheckPermission(UserId, PrivilegeType.Modify, ConstantPermission.Sys_GroupPermission);
            if (!isAccess)
            {
                return PartialView("AccessDenied");
            }
        ...
        {
            var isAccess = CheckPermission(UserId, PrivilegeType.Delete, ConstantPermission.Sys_GroupPermission);
            if (!isAccess)
            {
                return PartialView("AccessDenied");
            }

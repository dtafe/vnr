...
{
    public class Cat_BrandController : MainBaseController
    {
        ...
        public ActionResult CatBrandInfo(string id)
        {
            
            if (!string.IsNullOrEmpty(id))
            {
                var id1 = Common.ConvertToGuid(id);
                var service = new RestServiceClient<Cat_BrandModel>(UserLogin);
                service.SetCookies(Request.Cookies, _hrm_Hr_Service);
                var modelResult = service.Get(_hrm_Hr_Service, "api/Cat_Brand/", id1);
        ...
        {
            var service = new RestServiceClient<IEnumerable<Cat_BrandModel>>(UserLogin);
            service.SetCookies(Request.Cookies, _hrm_Hr_Service);
        ...
        public ActionResult RemoveSelected(string selectedIds)
        {
            ...
            var user = new List<Cat_BrandModel>();
            if (selectedIds != null)
            {
                var ids = selectedIds
                ...
                {
                    service.Delete(_hrm_Hr_Service, "api/Cat_Brand/", id);
                }
            }
            return Json("");
        }

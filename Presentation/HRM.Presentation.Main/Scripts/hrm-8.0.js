...
//[Hieu.Van] - Set biến tạm TempData 
function SetTempData(urlGetData, _TempData) {
    $.ajax({
        type: 'POST',
        url: urlGetData,
        data: { controller: _TempData },
        dataType: 'json',
        async: false,
        success: function (returnValue) {
        ...
        //error: function () {
        //    ShowNotificationProcessor('error,Lỗi Khi Gán Giá Trị TempData');
        //}
    });
}
...
function SelectcheckedNodeIds(nodes, checkedNodes) {
    ...
    if (arguments.length == 3) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (nodes[i].hasChildren) {
                SelectcheckedNodeIds(nodes[i].children.view(), checkedNodes);
            }
        }
    }
...
//[HIEN.Nguyen] - Auto check node by list Ordernumber
function CheckNodeByOrderNumber(nodes, listOrdernumber) {

    for (var i = 0; i < nodes.length; i++) {
        debugger;
        if (listOrdernumber.indexOf(nodes[i].OrderNumber.toString()) != -1) {
            nodes[i].checked = true;
        }
        if (nodes[i].hasChildren) {
            CheckNodeByOrderNumber(nodes[i].children.view(), listOrdernumber);
        }
    }
}

//[HIEN.Nguyen] - Xử lý check chọn nút trên treeview
...
//[HIEN.Nguyen] - Hàm check all cây phòng ban
function checkedAllNodeOrgtrusture(nodes, checkedNodes, checkedNodesName) {
    for (var i = 0; i < nodes.length; i++) {
        nodes[i].checked = true;
        $("input[type='checkbox'][value='" + nodes[i].id + "']").prop('checked', true);
        checkedNodes.push(nodes[i].id);
        checkedNodesName.push(nodes[i].Name);
        if (nodes[i].hasChildren) {
            checkedAllNodeOrgtrusture(nodes[i].children.view(), checkedNodes, checkedNodesName);
        }
    }
};
...
//}
// set language cho từng project
function SetLanguageHRM(lang, _hrm_Main_Web, _hrm_Sys_Web, UserLoginName) {
    $.ajax({
        url: _hrm_Main_Web + '/Setting/setLanguage/',
        data: { Language: lang, userName: UserLoginName },
        type: 'POST',
        async: false,
        datatype: 'json',
        success: function (result) { }
...
//[Hien.Nguyen] Hàm kiểm tra xem có chọn all page trên lưới hay không
//return 0 là không chọn All, return 1 là chọn all
function IsSelectAllPage() {
    return $('input[name=selectOnePageOrAllPage]:checked').val();
};
...
function CreateImportProgress(title, value) {
    $("#hrm_main_content").css("display", "none");

    $("#ModalsProgress").data("kendoWindow").open().title(title);
...
function EventDataBoundTreeViewOrg(e) {
    ...
    var listNode = Object;
    if (e.node != undefined && e.node[0] != undefined && e.node[0].children[1] != undefined) {
        listNode = e.node[0].children[1].children;
    }
    else {
        listNode = e.node;
    }
...
function EventDataBoundTreeViewOrgNoCheckBox(e) {
    ...
    if (listNode != undefined) {
        for (var i = 0; i < listNode.length; i++) {
            var dataItem = tree.dataItem(listNode[i]);
            if (!dataItem.IsShow) {
                if (listNode[i].childNodes[0].children[1] != undefined) {
...
//[Hien.Nguyen]
//Hàm xử lý truyền thêm Enum lên server của DropdownList
function FilterDataEnumDropdownList(e) {
    return {
        text: e,
    };
};
